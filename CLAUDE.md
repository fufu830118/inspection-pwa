...
### 2. 車輛檢查系統 (2025-12-30)
- **新增類別**：
  - **ID 20 主管座車** (每月檢查)：支援多台車輛歸戶於一位主管名下。
  - **ID 21 一般車輛** (每日檢查)：作業前檢點。
- **免掃碼模式**：這兩個類別在 `EquipmentList.vue` 中被設定為特例，允許直接點擊列表進入檢查表單，無需掃描 QR Code。
- **資料分組**：主管座車清單支援 `設備名稱` 欄位包含車號資訊 (例如 `William (RDM-8877 / ...)`)，實現一人檢查多車。
- **清單簡化**：設備清單 CSV 可省略 `類別ID` 與 `頻率` (由類別配置繼承)，並支援自訂欄位 (如 `品牌`)。

### 3. 數據結構優化 (雙軌制)
- **Sub-category 支援**：檢查紀錄現在會包含 `sub_category` (設備類型) 和 `device_name`，解決了區域檢查紀錄混雜的問題。
- **動態統計**：`Statistics.vue` 和 `History.vue` 已更新，能自動根據子類別拆分統計數據 (例如顯示「區域-咖啡機」而非籠統的「區域」)。
...
- **編碼要求**：所有 UTF-8 CSV 檔案 **必須包含 BOM** (`UTF-8 with BOM`)，以確保在 Microsoft Excel 中開啟時中文不亂碼。
- **欄位類型**：檢查項目 CSV 的類型欄位統一使用中文：`布林` (Checkbox), `簡答` (Textarea)。
- **特殊類別優化**：`csvLoader.js` 針對 **ID 16 (區域)** 進行優化，因為它是進入設備選擇頁，系統會自動跳過讀取 `檢點項目.csv`，無需建立空檔案。
...
#### 問題：CSV 解析欄位錯誤
**原因**：Excel 存檔帶有 BOM 或 欄位類型不匹配
**修正**：
1. `csvLoader.js` 已內建 BOM 移除功能。
2. 存檔時 **務必選擇 `UTF-8 with BOM`** 格式，確保 Excel 中文正常顯示。
3. 檢查項目類型請使用中文：`布林`、`簡答`。
